...
function parseInput(input) {
    ...
    const n = parseInt(lines[0]); // Число отрезков
    segments = [];
    for (let i = 1; i <= n; i++) {
    ...
    // Если координаты многоугольника присутствуют
    if (lines[n + 2]) {
...
function runAlgorithm() {
    ...
    segments.forEach(segment => {
        ...
        if (clipped) {
            console.log("Отсечённый отрезок:", clipped);
            // Нарисовать отсечённый отрезок
            const { x: x1, y: y1 } = transformCoordinates(clipped[0], clipped[1]);
            const { x: x2, y: y2 } = transformCoordinates(clipped[2], clipped[3]);
